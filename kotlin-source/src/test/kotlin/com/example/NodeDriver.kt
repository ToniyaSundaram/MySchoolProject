...
fun main(args: Array<String>) {
    ...
    driver(DriverParameters(isDebug = true, waitForAllNodesToFinish = true)) {
        val (partyA, partyB, partyC) = listOf(
                startNode(providedName = CordaX500Name("deo", "London", "GB"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("SchoolA", "New York", "US"), rpcUsers = listOf(user)),
                startNode(providedName = CordaX500Name("SchoolB", "Paris", "FR"), rpcUsers = listOf(user))).map { it.getOrThrow() }
